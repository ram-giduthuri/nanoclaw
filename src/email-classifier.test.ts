...
describe('classifyEmail', () => {
  it('keeps the actual review-request email', () => {
    const c = classifyEmail(
      gh(
        'review_requested',
        '@stefan requested your review on: repo#380 title.',
      ),
    );
  ...
  it('files thread noise that merely carries review_requested (merge/approve on a reviewed PR)', () => {
    ...
    expect(
      classifyEmail(gh('review_requested', 'Merged #435 into main.'))
        .disposition,
    ).toBe('file');
    expect(
      classifyEmail(
        gh('review_requested', '@harald approved this pull request.'),
      ).disposition,
    ).toBe('file');
    expect(
      classifyEmail(gh('review_requested', 'Quality Gate passed')).category,
    ).toBe('GitHub');
  });

  it('keeps a fresh direct @mention, files mention-tagged thread noise', () => {
    expect(
      classifyEmail(gh('mention', '@ram-giduthuri could you take a look?'))
        .disposition,
    ).toBe('keep');
    // A sonar bot comment on a thread you were mentioned in — no fresh @mention.
    expect(
      classifyEmail(gh('mention', 'sonarqube-tereina[bot] Quality Gate passed'))
        .disposition,
    ).toBe('file');
  });
  ...
  it('files Jira and Confluence with their own tags', () => {
    expect(
      classifyEmail({ fromAddress: 'jira@tereina.atlassian.net' }).category,
    ).toBe('Jira');
    expect(
      classifyEmail({ fromAddress: 'confluence@tereina.atlassian.net' })
        .category,
    ).toBe('Confluence');
  });
  ...
  it('keeps Jira/Confluence "mentioned you" in the inbox, files routine activity', () => {
    expect(
      classifyEmail({
        fromAddress: 'jira@tereina.atlassian.net',
        subject: '[JIRA] Stefan Arnold mentioned you on DA-435',
      }).disposition,
    ).toBe('keep');
    expect(
      classifyEmail({
        fromAddress: 'jira@tereina.atlassian.net',
        subject: '[JIRA] (PAYR-145) status changed',
      }).disposition,
    ).toBe('file');
    expect(
      classifyEmail({
        fromAddress: 'confluence@tereina.atlassian.net',
        subject: 'Brady mentioned you on a page',
      }).disposition,
    ).toBe('keep');
  });
  ...
  it('flags automated + unknown keeps for the review tag, not confident keeps', () => {
    expect(
      classifyEmail({ fromAddress: 'newsletter@company.com' }).uncertainKeep,
    ).toBe(true); // automated
    expect(
      classifyEmail({ fromAddress: 'system@randomvendor.io' }).uncertainKeep,
    ).toBe(true); // unknown, no display name
    expect(
      classifyEmail({
  ...
  it('files non-GitHub CI bots with the CI tag', () => {
    expect(
      classifyEmail({ fromAddress: 'no-reply@sonarqube.example.com' }).source,
    ).toBe('ci');
    expect(classifyEmail({ fromAddress: 'ci@company.com' }).source).toBe('ci');
    expect(
      classifyEmail({ fromAddress: 'notifications@circleci.com' }).category,
    ).toBe('CI');
  });
  ...
  it('does NOT mistake a human with a CI-like surname for a CI bot', () => {
    ...
    const c = classifyEmail({
      fromAddress: 'sarah.jenkins@tereina.com',
  ...
  it('keeps calendar/meeting messages untouched', () => {
    ...
    expect(
      classifyEmail({ fromAddress: 'x@y.com', subject: 'Canceled: Standup' })
        .source,
    ).toBe('calendar');
  });
  ...
  it('keeps GitHub security/auth mail (noreply@github.com) — never files a 2FA code', () => {
    ...
    const token = classifyEmail({
      ...
      fromName: 'GitHub',
      subject:
        '[GitHub] A personal access token (classic) has been regenerated',
    });
  ...
  it('keeps unknown senders (default-safe)', () => {
    const c = classifyEmail({
      fromAddress: 'dse_NA4@docusign.net',
      fromName: 'DocuSign',
    });
    expect(c.disposition).toBe('keep');
  });
...
describe('decideAction', () => {
  ...
  it('files only when live AND the source is enabled', () => {
    expect(
      decideAction(fileCls, { dryRun: false, fileSources: ['github'] }),
    ).toEqual({ file: true, tag: false });
    expect(
      decideAction(fileCls, { dryRun: false, fileSources: ['jira'] }).file,
    ).toBe(false); // source off
    expect(
      decideAction(fileCls, { dryRun: true, fileSources: ['github'] }).file,
    ).toBe(false); // dry-run
    expect(decideAction(fileCls, { dryRun: false, fileSources: [] }).file).toBe(
      false,
    ); // fail-safe default
  });
  ...
  it('tags an uncertain keep only when live, never files it', () => {
    expect(
      decideAction(uncertainCls, { dryRun: false, fileSources: [] }),
    ).toEqual({ file: false, tag: true });
    expect(
      decideAction(uncertainCls, { dryRun: true, fileSources: [] }).tag,
    ).toBe(false);
  });

  it('does nothing for a confident keep (human)', () => {
    expect(
      decideAction(humanCls, {
        dryRun: false,
        fileSources: ['github', 'jira'],
      }),
    ).toEqual({ file: false, tag: false });
  });
});
